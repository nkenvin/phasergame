...
export default class PostGameHighscoresScene extends Phaser.Scene {
  ...
  create() {
    this.playerSettings = {
      playerName: this.sys.game.globals.playerName,
      score: this.sys.game.globals.score,
    };

    
    
    this.highscore = this.add.text(config.width / 2, 100, `${this.playerSettings.playerName}\nFinal Score: ${this.playerSettings.score}`, {
      fontFamily: 'Arial',
      fontSize: '56px',
      color: '#00FF33',
      align: 'center',
    ...
    const highScoreText = this.add.text(config.width / 2, 200, 'Best Score: ', {
      font: '56px Arial',
      color: '#00FF33',
      align: 'center',
      lineHeight: '1.5',
    ...
    highScoreText.setText('Best Score: ' + localStorage.getItem("highscore"));
    {
     if (this.playerSettings.score > localStorage.getItem("highscore")) 
        { 
            localStorage.setItem("highscore", this.playerSettings.score);
        }
    }

   
    // postHighscores(this.playerSettings.playerName, this.playerSettings.score).then(response => {
    ...
    this.nextButton = new ButtonGen(this, config.width / 2, config.height / 2 + 300, 'nextButton', 'nextButtonFocus', 'GameOver', this.sfx.btnSelect);
